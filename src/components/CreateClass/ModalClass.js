...
function ModalClass({titleClass}){
    ...
    document.addEventListener('click', function(event) {
        if(event.target.className === 'nav-close' || event.target.className === 'opacity-manage' || event.target.className === 'btn-save-edit-class' || event.target.className === 'btn-edit-class-close' ){
            setForm(false);
        }
      });
